import { useGetAllPermissionsQuery, useGetAllUserPermissionsQuery } from '@/store/dataApi/dashboard/permissionApi';
import {  useGetUserQuery, useUpdateUserMutation,  } from '@/store/dataApi/dashboard/usersApi';
...
const EditPills =(props)=>{
  ...
  const {data:userPermission}=useGetAllUserPermissionsQuery(props?.id)
  const {data:userdata}=useGetUserQuery(props?.id)
  console.log(userdata)
  console.log(userPermission?.data)

  const userper = [userPermission?.data?.map(val=>{
        ...
        <div
          ...
          data-te-tab-active>
            ...
            <div className="flex flex-col my-2">
                {/* user name */}
                <input id="categoryName" value={inputs.name} onChange={InputHandler} name='name' type="text" className="bg-gray-100 border-2 p-2 focus:outline-2 focus:outline-sky-400" placeholder="User Name"/>
            </div>
            <div className="flex flex-col my-2">
                {/* user name */}
