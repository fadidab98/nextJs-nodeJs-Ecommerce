...
import { getAllProduct, getRunningQueriesThunk, useDestroyProductMutation, useGetAllProductQuery } from "@/store/dataApi/dashboard/productApi"
...
const index = (props)=>{
    ...
    const data1 =[{title:'Product Name'},{title:'Category'},{title:'status'},{title:'Control'}]
    return <AppLayout>
        <Head>
            <meta charset="UTF-8"/>
     ...
     {/* header */}
        <Header father="Products" child="Home"/>
     {/* Ende header */}
     {/* Table */}
               ...
               return <tr key={data.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center">
                    {permissions.indexOf("show-product") !==-1 ?(<Link className=" p-2 rounded-md bg-sky-800 text-md text-white mx-1" href={`/dashboard/products/show/${data.id}`} >Show</Link>):('')}
                  {permissions.indexOf("delete-product") !==-1 ?(<button onClick={()=>{destroyProduct({id:data.id}).then(()=>router.replace(router.asPath))}} className=" p-2 rounded-md bg-red-800 text-md text-white mx-1" >Delete</button>):('')}
...
console.log('ref',JSON.stringify(ref.refreshToken))
  await axios.post('http://localhost:8800/api/refresh',{refreshToken}).then(res=>{
    console.log('refreshToken',res.data.refreshToken)
      setCookie('refreshToken',res.data.refreshToken,{ctx,maxAge: 60 *60 *1000})
      refresh = res.data.refreshToken;
      setCookie('access_token',res.data.access_token,{ctx,maxAge: 24 *60 *60 *1000})
    ...
    await store.dispatch(getAllProduct.initiate({page:page,cookie:token}));
    await Promise.all(store.dispatch(getRunningQueriesThunk()))


    const {data:products , status} = getAllProduct.select({page:page,cookie:token})(store.getState());
    ...
    return {
        props:{data:products?products: null ,tokens:tokens,refresh:refresh,permission:permi}
    }}
